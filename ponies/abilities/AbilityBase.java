...
public abstract class AbilityBase
{
	...
	public AbilityBase(String name, Race race, int cooldown)
	{
		this.name = name;
	...
	public AbilityBase(String name, Race race, int cooldown, int casttime)
	{
		this.name = name;
		icon = name.toLowerCase().replace(' ', '\0');
		this.race = race;
	...
	public static AbilityBase[] NewAbilityArray()
	{
		ArrayList<AbilityBase> abilityList = new ArrayList<AbilityBase>();
		for(Class c : abilityClasses)
		{
			try {
				AbilityBase ability = (AbilityBase)c.getConstructor().newInstance();
				abilityList.add(ability);
			} catch (Exception e) {e.printStackTrace();}
		}
		
		return abilityList.toArray(new AbilityBase[0]);
	}
